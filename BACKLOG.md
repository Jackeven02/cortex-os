...
> 1. ~~**`restore` does not auto-run.** `checkpoint.restoreAs` mints the restored process in state `NEW` (correct per PROCESS.md §3.6), and the dispatcher enqueues it, but the scheduler only dispatches `READY` and `reconcile()` only adopts `READY`, so a restored process sat in `NEW` until something walked it forward.~~ **Resolved in `0.2.0`:** `dispatcher.#restore` now walks NEW → READY and enqueues before returning, so a restored process is runnable like any spawned one and the CLI stopgap (`cortex restore` doing the walk itself) is deleted. `restoreAs` still mints NEW — adoption is the syscall's job, not the manager's.
> 2. ~~**Sync syscalls skip per-call recording.** `now` / `random` / `budget` / `on_signal` are synchronous on `CortexContext` (ABI §8) but `dispatcher.invoke` is uniformly async, so boot's sync fast-path replicated the state gate (`ESTATE`) yet appended no `.crec` record — replay had to trust that the injected clock/RNG would produce the same values.~~ **Resolved in `0.2.0`:** `dispatcher.noteSyncCall` builds the frame at call time (capturing the value actually served) and the dispatcher flushes the queue at the start of the process's next `invoke`, so the frames land before that syscall's `enter` and the log stays in causal order. **`budget` remains unrecorded — now by explicit policy (ABI §4.8), because it is derivable from the syscalls that spent it**, not because the plumbing cannot reach it. `on_signal` records the disposition kind, never the handler closure.
...
> 4. ~~**`sleep()` does not park.** `dispatcher.#sleep` awaited a real timer while the process stayed RUNNING the whole time, so `ps` called a napping agent runnable.~~ **Resolved in `0.2.0`:** `#sleep` now walks RUNNING → BLOCKED (`blockedOn.kind === 'sleep'`) → READY and wakes through the wake gate, so the body resumes only after a re-dispatch; `exit` cancels a pending sleep timer. `blockedOn` gained a `sleep` variant, which is why ABI §4.6 and the state model moved in the same commit.
...
- [x] **#044** Landing page (`cortex.sh` or similar — domain to be checked)
  - Built as **`docs/index.html`** — a single self-contained, dependency-free page (no CDN, no build step) so it is servable directly by **GitHub Pages from `/docs`** with zero configuration. Sections: hero + an animated replay of the Demo B reboot transcript; the "function vs process" contrast that is the whole thesis; a primitive grid for the twelve CLI verbs; the three killer demos with links to their live replays (`demo-a/b/c.html`); a quickstart (clone → offline mock → supervision tree) beside a real agent snippet; and an honest status panel (11 modules / 19 syscalls / 7 drivers / 479 smoke checks) with the v0 caveats stated plainly. A custom domain (`cortex.sh`) is still to be decided/registered — the page works on `github.io` today.
...
- [x] **#047** HN launch post draft
  - `docs/launch/hn.md` — three title options, a "Show HN" body that leads with the function-vs-process thesis, states exactly what runs today (offline, mock driver), walks the three demos, volunteers the v0 gaps *before* the commenters do, and pre-answers "why not Temporal/LangGraph/k8s?". Plus posting notes: seed the thread with the Demo B transcript (many readers won't click through), have the Erlang / TypeScript / kernel-LOC answers ready, and don't use "revolutionary"/"10x".
- [x] **#048** X / V2EX / 即刻 launch post drafts
...
- [x] **#050** Logo (something evocative of layers, nuclei, or branching — not a brain clip-art)
  - Shipped as vector SVG in [`assets/`](./assets), plus [`docs/logo.md`](./docs/logo.md) with the rationale and usage rules. The mark is one idea, not three: two concentric **layer** arcs (a cortex is a surface, so the arcs are open, not a filled blob) whose opening at the upper right emits a **fork** — a process tree ending in filled **nucleus** nodes, one of which sits at the centre. Layers → nucleus → branch. Six files: `logo.svg` (light), `logo-dark.svg` (dark), `logo-mono.svg` (`currentColor`, for stamps/print), `logo-tile.svg` (512² rounded tile with a baked-in background, for avatars), `favicon.svg` (simplified — inner arc dropped and strokes thickened so it survives 16px), and `wordmark.svg`. All are script-free, `<style>`-free, reference-free SVG, so GitHub renders them inline in Markdown. Wired into both READMEs (header image + doc-table row), the landing page nav/footer/hero (replacing the old placeholder dot), and the landing page `<link rel="icon">` (replacing a stale red-circle data-URI). `package.json` `files` now ships `assets/` and the previously-missing `MANIFESTO.zh-CN.md`.
