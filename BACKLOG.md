...
> **Status as of this commit:** 7 of 10 items done. The foundation (`types.ts` + `errors.ts`) plus `recorder.ts`, `process_table.ts`, `signals.ts`, `ipc.ts`, `memory.ts`, and `checkpoint.ts` have landed; 143 smoke checks green. Next: `fork.ts` (#020), `scheduler.ts` (#013), `init.ts` (#021), and finally `syscall.ts` (#014) which wires everything together.
...
- [x] **#017** `kernel/checkpoint.ts` — `checkpoint`, `restore`, snapshot serialization (CBOR) → this commit
