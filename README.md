...
**Released as [`v0.1.2`](./CHANGELOG.md)** (2026-09-19) — a documentation-only patch on `v0.1.1`, which was a one-defect patch on `v0.1.0`, the first tagged release, covering Phases 0–5. The `0.x` is honest: the syscall ABI is not frozen until `1.0.0`, so a minor bump may carry a breaking change to the ABI or the state model. If you build against Cortex today, pin the exact version. (Phases are build milestones; the versions are the releases.)
